...
| The instrument pin: default, persistence, URL precedence, night mode hidden for LATISS, the *same real dataId* resolving to shutter closes an hour apart on the two instruments, a resolved t₀ dropped when the pin changes — including on re-entry to home, where the pin can have moved under the page — a pinned view excluding the other instrument's pods, Tonight filtering and link-carrying | `test_instrument.py` |
...
| The cache table's contents and links — including that an exposure row's link carries the instrument its window was fetched under, so it cannot open the same-numbered twin — single-window and whole-cache deletion (checked on disk, not just in the table), the confirmation being declinable, a deleted window booting its loaded view home, and the live night dir being absent from the listing | `test_admin.py` |
