...
| Area | File |
|---|---|
| URL → view routing, the base path (every asset and API call carrying the prefix, paths outside it 404ing, and a whole form → fetch → timeline run staying under the prefix end to end), the browser history the app pushes for itself (Back from a fetched view returning to the form rather than out of the application, Forward returning to it, Back undoing the explore and night views' home buttons, the `autoFetch` drilldown entry being rewritten rather than added to so Back can't re-fire the fetch, and entries carrying the base path), the FAQ overlay, the tab icon and the topbar logo actually rendering (`naturalWidth`, so a broken image fails rather than passing as a gap on the page) in each of the four views whose headers are separate markup, with the prefix substituted into both | `test_shell.py` |
| Two tabs at once: two exposures, a night beside an exposure, and the same dataId under each instrument all held independently across reloads; Back in one tab leaving the other alone; an admin cache-row link opening a real second tab under a base path; and a tab whose server-side state a later tab evicted still rebuilding from the cache when Back lands on it | `test_tabs.py` |
...
| Restart inference   | The real isolated in-place restart k8s's `count` cannot see (20260813 SFM worker: 46 min of work, stopped mid-`isr`, fresh container 7 s later) promoted from `POD_STARTED` to `POD_RESTARTED`, with the inference named in its message — plus each false-positive shape it must refuse: an init container's preamble, a rescheduled StatefulSet pod whose file holds its predecessor's lines, and a pod that logged and then went quiet. `readPodLifecycle` returning the sandbox times `classifyK8sEvent` drops | `tests/test_parse.py`             |
| Night views         | `_nightViewFromBody` (default / normalise / reject) and `_nightViewFromQuery` (falls back rather than 400s, since a person types it); `_nightPodFilter` `None` for AOS so Loki's filter isn't re-implemented; the SFM spec pushing **no** regex down; both halves of one dayObs held at once, evicted independently, and a bare dayObs still meaning AOS; the payload carrying its `view`. Endpoint side: an unknown view 400s, the SFM job reports `nightView` and asks Loki for every pod, and `/api/summary` serves the half it was asked for | `tests/test_server.py`, `tests/test_server_endpoints.py` |
...
| Pod classification | `podGroup` (longest-prefix-match, order-independence regression, full real-pod fixture parametrisation), `podOrdinal`, `podInstrument`, `groupLabels` (defensive-copy contract); `isAosPod` pinned against `NIGHT_AOS_POD_REGEX` over real pod names, since the night's two views are complements and a drift between them loses a pod silently; `summarizeAll(keep=…)` filtering before parsing | `tests/test_parse.py`             |
...
| Packaging          | The container installs the package rather than running from the source tree, so `[tool.setuptools.package-data]` decides what exists in production: every `__BASE_PATH__/static/...` asset the template asks for is on disk *and* selected by those globs (a `static/img/x.png` under a `static/*` glob would serve fine on a laptop and 404 in the deployment), no static asset grows past 200 KiB (what would notice full-size source art committed over a served copy), and every `<img>`'s `width`/`height` match the PNG's real intrinsic size — they reserve the box before the bytes arrive, and go stale silently the moment the art is regenerated at another size | `tests/test_packaging.py` |
| Browser (UI)       | Chromium against the real app over a real cut-down night — routing, browser history and multi-tab behaviour, the base path, the instrument pin, the timeline, the night view, the forms and fetch flow, the Tonight panel, the cache admin table, and the range navigator. See *Browser tests* above for the breakdown | `tests/ui/` |
